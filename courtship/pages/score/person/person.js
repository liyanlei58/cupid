...
var scoreFormIdDb = require('../../js/scoreFormIdDb.js')
...
Page({
  ...
  score: function(e) {
    ...
    if(score == "" || score == 0){
      common.toastError("请打分");
      return
    }
    var formId = e.detail.formId;
    //存入数据库
    var scoreFormId = {
      activityId: that.data.activity._id,
      formId: formId
    }

    //从数据库中取出
    scoreFormIdDb.getListByActIdAndPersonId(that.data.activity._id, app.globalData.openid, function (dataList) {
      if (dataList.length == 0) {
        //没有值
        scoreFormIdDb.addOne(scoreFormId, function () { });
        return
      }
      //有值
      scoreFormIdDb.updateOne(scoreFormId, dataList[0]._id, function () { });
    })
  ...
  sendOneMsg: function (fromUser, toUser, tips) {
    ...
    var index = Math.ceil(Math.random() * matchDb.matchMsgArray.length);
    //从数据库中取出
    scoreFormIdDb.getListByActIdAndPersonId(that.data.activity._id, toUser._openid, function(dataList){
      if(dataList.length == 0){
        //没有值
      ...
      //发送匹配消息
      msg.sendTemplateMsg(toUser._openid, msgObj, function (res) {
